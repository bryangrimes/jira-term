...
(function() {
  ...
  addItem = function(project) {
    return dutils.ask("Summary", /.+/, function(summary) {
      return dutils.ask("Description", /.+/, function(description) {
        return dutils.ask("QA Aceptance Criteria", /.+/, function(criteria) {
          return jiraCli.getIssueTypes(function(issueTypes) {
            ...
            addIssueCallback = function(type) {
              return jiraCli.addIssue(summary, description, issueTypes[type - 1].id, project, criteria);
            };
  ...
  if (require.main === module) {
    ...
    }).usage('Usage:\n\tjira -f EG-143\n\tjira -f EG-143 -d\n\tjira -l').boolean('d').string('s').string('f').string('t').string('w');
    if (argv.argv.help) {
      argv.showHelp();
      return;
    }
